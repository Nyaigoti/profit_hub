...
export function EvenOddTab({
    ...
    const changeRate =
        recentDigits.length > 1
            ? (recentDigits.filter((d, i) => i > 0 && d % 2 !== recentDigits[i - 1] % 2).length / recentDigits.length) *
              100
            : 0;

    const reversalRate =
        recentDigits.length > 1
            ? (recentDigits.filter((d, i) => i > 0 && d % 2 !== recentDigits[i - 1] % 2).length /
                  (recentDigits.length - 1)) *
              100
            : 0;
        ...
        <div className='space-y-6'>
            ...
            >
                ...
                {currentDigit !== null ? (
                    ...
                    <div className={`text-4xl font-bold ${theme === 'dark' ? 'text-gray-600' : 'text-gray-400'}`}>
                        -
                    </div>
                )}
            ...
            >
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-8 mb-8'>
                    ...
                    >
                        <div
                            className={`text-5xl font-bold mb-2 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}
                        >
                        ...
                        <div
                            className={`text-sm mt-2 font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-700'}`}
                        >
                    ...
                    >
                        <div
                            className={`text-5xl font-bold mb-2 ${theme === 'dark' ? 'text-pink-400' : 'text-pink-600'}`}
                        >
                ...
                >
                    <div
                        className={`text-xl font-semibold mb-3 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-800'}`}
                    >
                        Entry Condition:
                    </div>
                    {signalStatus === 'TRADE NOW' ? (
                        <>
                            ...
                            <div
                                className={`text-lg font-medium ${theme === 'dark' ? 'text-green-400' : 'text-green-700'}`}
                            >
                                {entryMessage}
                            </div>
                        </>
                        ...
                        <>
                            <div
                                className={`text-2xl font-medium mb-4 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}
                            >
                                {entryMessage}
                            </div>
                            <div
                                className={`text-base font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-700'}`}
                            >
            ...
            >
                ...
                >
                    <p className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-800'}`}>
                        <strong>Explanation:</strong> Pattern recognition analyzes the behavior of even and odd digits
                        over time. A high change rate indicates frequent alternation between even and odd, while a high
                        reversal rate shows the tendency to switch after consecutive same-type digits. Use this data to
