...
export function LastDigitsLineChart({ digits }: LastDigitsLineChartProps) {
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [digits]); // Re-bind observer if digits change (though draw is closed over digits, so we need to be careful. Better to just call draw inside observer)
    // Actually, if we put draw inside the effect or use useCallback, it's cleaner.
    // But draw depends on digits.
    // Let's just make draw a useCallback dependent on digits.

    return (
        <div ref={containerRef} className='w-full h-48 sm:h-56 md:h-64 bg-gray-200 dark:bg-gray-800/50 rounded-lg p-4'>
