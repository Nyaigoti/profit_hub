...
export function useDerivAuth() {
    ...
    useEffect(() => {
        ...
        if (acct1 && token1) {
            console.log('[v0] 🔐 OAuth callback detected');
            ...
            while (urlParams.get(`acct${i}`)) {
                ...
                // Store tokens securely (in a real app, consider HttpOnly cookies or more secure storage)
                // For this implementation, we'll stick to localStorage as per existing pattern
                if (i === 1) {
        ...
        if (storedToken && storedToken.length > 10) {
            ...
            // Don't show modal immediately to avoid annoying user if they are using DBOT auth
